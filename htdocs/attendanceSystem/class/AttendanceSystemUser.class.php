...
class AttendanceSystemUser extends CommonObject
{
    ...
    function fetchDuplicate()
    {
        ...
        $sql .= " WHERE ";
		$sql .= ' as_name '.(empty($this->as_name)?'is NULL':"= '".$this->db->escape($this->as_name)."'").' AND ';
		$sql .= ' as_uid '.(empty($this->as_uid)?'is NULL':"= '".$this->as_uid."'").' AND ';
		$sql .= ' rfid '.(empty($this->rfid)?'is NULL':"= '".$this->rfid."'").' AND ';
		$sql .= ' role '.(empty($this->role)?'is NULL':"= '".$this->role."'").' AND ';
		$sql .= ' passwd '.(empty($this->passwd)?'is NULL':"= '".$this->db->escape($this->passwd)."'").' AND ';
		$sql .= ' status '.(empty($this->status)?'is NULL':"= '".$this->status."'").' AND ';
        $sql .= ' mode '.(empty($this->mode)?'is NULL':"= '".$this->mode."'");
        $sql .= " LIMIT 1 ";
		dol_syslog(__METHOD__, LOG_DEBUG);
        $resql = $this->db->query($sql);
	...
	global $conf, $langs;
        ...
        if($id){
            $lien = '<a href = "'.dol_buildpath('/attendanceSystem/AttendanceSystemUserCard.php',1).'?id='.$id.'&action=view"'.$linkclose.'>';
        }else if (!empty($ref)){
            $lien = '<a href = "'.dol_buildpath('/attendanceSystem/AttendanceSystemUserCard.php',1).'?ref='.$ref.'&action=view"'.$linkclose.'>';
        }else{
            $lien = "";
        }
