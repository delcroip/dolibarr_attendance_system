...
$attendancesystemeventStatusArray = array(0 => 'loaded', 1 => 'in', 2 => 'out', 3 => 'ErrorDur' , 4 => 'ErrorDouble', 5 => 'ErrorIn', 6 => 'ErrorOut');
/**
...
class AttendanceSystemEvent extends CommonObject
{
	...
	global $conf, $langs;
        ...
        if($id){
            $lien = '<a href = "'.dol_buildpath('/attendanceSystem/AttendanceSystemEventCard.php',1).'?id='.$id.'&action=view"'.$linkclose.'>';
        }else if (!empty($ref)){
            $lien = '<a href = "'.dol_buildpath('/attendanceSystem/AttendanceSystemEventCard.php',1).'?ref='.$ref.'&action=view"'.$linkclose.'>';
        }else{
            $lien = "";
        }
...
// Commit or rollback
  ...
  * @return int id of tasktime created
  */
