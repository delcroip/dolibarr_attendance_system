...
$langs->loadLangs(array("AttendanceSystem@attendanceSystem", "companies", "other"));
...
//$result = restrictedArea($user, 'attendanceSystem', $id);
...
$diroutputmassaction = $conf->attendanceSystem->dir_output . '/temp/massgeneration/'.$user->id;
...
if($object->id) {
        ...
        dol_fiche_head($head, 'document', $langs->trans("AttendanceSystem"), -1, 'AttendanceSystem@attendanceSystem');
        ...
        // Object card
        // ------------------------------------------------------------
        $linkback = '<a href = "' .dol_buildpath('/attendanceSystem/AttendanceSystemAdmin.php', 1) . '?restore_lastsearch_values=1' .(! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
    ...
    print '<div class = "underbanner clearboth"></div>';
        ...
        print '</div>';
        dol_fiche_end();
        $modulepart = 'attendanceSystem';
        //$permission = $user->rights->attendanceSystem->create;
        $permission = 1;
        //$permtoedit = $user->rights->attendanceSystem->create;
        $permtoedit = 1;
        $param = '&id=' . $object->id;
